...
const Card = ({ name, description, degree, image, aos = "zoom in" }) => {
  ...
  rounded-2xl overflow-hidden shadow-lg hover:shadow-green-900/40 
  p-6 text-center border border-neutral-700"
      ...
      >
        <img
          src={image}
          alt={name}
          className="w-28 h-40 drop-shadow-lg drop-shadow-green-600  mx-auto mb-4 "
        />
        <h3 className="text-xl font-bold text-gray-100">{name}</h3>
        <p className="text-green-400 font-medium mb-3">{degree}</p>
